...
def generate_samples(files, positive_only = True):
    assert(len(files))
    search_terms = []
    content = []
    for f in files:
        ...
        for row in datachunk:
            ...
            number_to_duplicate_description = len(row['correctlyspelled_search_terms']) + len(row['misspelled_search_terms'])
            if not positive_only:
                number_to_duplicate_description += len(row['wrong_search_terms'])
